...
export interface RouteData {
  nodeIds: string[];
  linkIds: string[];
  segmentRoutes?: SegmentRoute[]; // SegmentRoute는 아래에 정의
}
...
// 서버 경로 응답 (지도 표시에 사용될 수 있음) - RouteData와 필드명을 맞추고 타입을 더 명확히
export interface ServerRouteResponse {
  nodeIds: (string | number)[]; // 서버에서는 숫자로 올 수 있으므로 유지
  linkIds: (string | number)[]; // 서버에서는 숫자로 올 수 있으므로 유지
...
export interface ParsedRouteData {
  day: number;
  segments: RouteSegmentForParsedData[]; // 아래 정의된 RouteSegmentForParsedData 사용
  totalDistanceMeters: number;
}

// ParsedRouteData 내부의 RouteSegment 타입 (위의 SegmentRoute와 구분)
...
export function convertPlannerResponseToNewResponse(
  ...
  const routeSummaryItems: ServerRouteSummaryItem[] = plannerResponse.map(item => {
    ...
    return {
      ...
      status: 'OK',
      total_distance_m: 0,
      interleaved_route: item.nodeIds, // 서버 응답이 숫자 배열이므로 그대로 사용
      places_scheduled: [], // 기본값
      places_routed: [],    // 기본값
    };
  });
  ...
  plannerResponse.forEach(dayData => {
    dayData.nodeIds.forEach((nodeId, index) => {
      ...
      if (index % 2 === 0 && typeof nodeId === 'number') { 
        scheduleItems.push({
          id: nodeId, // 서버에서 숫자로 오므로 그대로 사용
          place_name: `장소 ${nodeId}`, // 실제 장소 이름 매핑 필요
          place_type: 'unknown', // 실제 장소 타입 매핑 필요
          time_block: '시간 정보 없음', // 실제 시간 정보 매핑 필요
        });
      }
    });
  });
...
// Raw Data Types (추가됨 from original index.ts)
...
// 서버 응답 파싱 유틸리티 타입 (추가됨 from original index.ts)
