...
// 세그먼트 경로 인터페이스 (core.ts 내에서 중복 정의 피하기 위해 이름 변경 또는 선택적 사용)
// 이미 아래에 RouteSegment 이름으로 정의되어 있으므로, 여기서는 주석 처리하거나 이름을 다르게 해야 합니다.
// 만약 동일한 구조라면 하나만 남깁니다. 여기서는 아래의 RouteSegment를 사용한다고 가정합니다.
/*
...
// RouteSegment와 SegmentRoute가 구조가 다르므로 둘 다 유지할 수 있습니다.
// 사용자 프롬프트에서는 SegmentRoute가 RouteData 내부에 있고, RouteSegment는 ParsedRouteData 내부에 있었습니다.
// 프롬프트에서 제공된 SegmentRoute (fromIndex, toIndex 포함)
export interface SegmentRoute {
    fromIndex: number;
    toIndex: number;
    nodeIds: string[];
...
// 프롬프트에서 제공된 RouteSegment (from, to, links 포함)
...
export interface ParsedRouteData {
  day: number;
  segments: RouteSegment[]; // 위의 RouteSegment 사용
